...
struct SwiftGlobalTable {
    ...
    ArrayOfPtrTo<ArrayOfPtrTo<SwiftMeta>> buckets;
...
public:
    ...
    SwiftMeta(MetaNames names, PtrTo<ModuleMeta> m, uint16_t flags){
        _names = names;
        _topLevelModule = m;
        _flags = flags;
    }
...
template <>
struct PtrTo<ArrayOfPtrTo<SwiftMeta>> {
    int32_t offset;
...
//        return add(value);
//    }
    const ArrayOfPtrTo<SwiftMeta>* operator->() const {
        return valuePtr();
    }
...
//    }
    ...
    const ArrayOfPtrTo<SwiftMeta>* valuePtr() const {
        return isNull() ? nullptr : reinterpret_cast<const ArrayOfPtrTo<SwiftMeta>*>(tns::offset(SwiftMetaFile::instance()->heap(), this->offset));
    }
    const ArrayOfPtrTo<SwiftMeta>& value() const {
        return *valuePtr();
    }
};
...
template <>
struct PtrTo<SwiftMeta> {
    int32_t offset;

    bool isNull() const {
        return offset == 0;
    }
    PtrTo<SwiftMeta> operator+(int value) const {
        return add(value);
    }
    const SwiftMeta* operator->() const {
        return valuePtr();
    }
    PtrTo<SwiftMeta> add(int value) const {
        return PtrTo<SwiftMeta>{ .offset = static_cast<int32_t>(this->offset + value * sizeof(SwiftMeta)) };
    }
    PtrTo<SwiftMeta> addBytes(int bytes) const {
        return PtrTo<SwiftMeta>{ .offset = this->offset + bytes };
    }
    template <typename V>
    PtrTo<V>& castTo() const {
        return reinterpret_cast<PtrTo<V>>(this);
