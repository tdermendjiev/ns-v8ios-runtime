...
in_port_t VoivodaServer::Init(std::function<void (std::function<void (std::string)>)> onClientConnected, std::function<void (std::string)> onMessage) {
    in_port_t listenPort = 18184;
    ...
    dispatch_queue_t q = dispatch_queue_create("voivoda_server_queue", DISPATCH_QUEUE_CONCURRENT);
            ...
            if ((clientSocket = accept(serverSocket, (struct sockaddr *) &echoClntAddr, &clntLen)) >= 0) {
                ...
                __block dispatch_io_handler_t receiver = ^(bool done, dispatch_data_t data, int error) {
                    ...
                    dispatch_io_read(channel, 0, length, q, ^(bool done, dispatch_data_t data, int error) {
                        ...
                        if (size) {
                            NSString* payload = [[NSString alloc] initWithData:(NSData*)data encoding:NSUTF8StringEncoding];
...
void VoivodaServer::Send(dispatch_io_t channel, dispatch_queue_t queue, std::string message) {
